<template>
  <div class="container mt-2" v-if="tasks.length != 0">
    ...
    <b-modal ref="modalRemove" hide-footer title="Eliminacion de propiedad">
      <div class="d-block text-center">
        <p>Desesa eliminar esta propiedad?</p>
        <p>Propiedad: {{ taskSelected.subject }}</p>
      </div>
      <div class="mt-3 d-flex justify-content-end">
        <b-button variant="outline-secondary" class="mr-2" @click="hideModal"> Cancelar </b-button>
        <b-button variant="outline-danger" class="mr-2" @click="confirmRemoveTask"> Borrar </b-button>
      </div>
    </b-modal>
  </div>
  <div class="container mt-2 alinhar" v-else>
    <b-card class="mb-2" id="card1">
      <b-card-text>No tienes ninguna propiedad</b-card-text>
      <b-card-text>Crea una</b-card-text>
      <b-button variant="outline-danger" class="mr-2" to="/form">Click aqui!</b-button>
    </b-card>
  </div>
</template>
